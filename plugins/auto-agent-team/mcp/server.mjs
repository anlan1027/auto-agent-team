...
const SERVER_VERSION = "0.3.0-dev.2";
...
function uninitializedDashboardState(root) {
  return {
    ...
    members: [],
    tasks: [],
    events: [{ at: now(), kind: "runtime_uninitialized", message: `No Agent Team state exists yet at ${statePath(root)}. Waiting for agent_team_create.`, memberId: null, taskId: null }]
  };
}
...
function reconcileMembers(state) {
  ...
  if (allTasksDone) {
    for (const member of state.members) {
      ...
      if (member.status !== "done" || member.currentTask !== null || member.summary !== "Team completed") {
        member.status = "done";
...
const tools = [
  {
    ...
    description: "Create or replace DSH-style Agent Team state for a local workspace. Start executionMode as UNKNOWN unless native delegation or fallback has already been proven.",
    inputSchema: { type: "object", properties: {
      workspacePath: { type: "string" }, name: { type: "string" }, description: { type: "string" },
  ...
  },
  {
    ...
    description: "Read and reconcile current Agent Team state. Returns initialized=false when no state exists yet.",
    inputSchema: { type: "object", properties: { workspacePath: { type: "string" } }, required: ["workspacePath"] },
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false }
  },
  {
    name: "agent_team_set_execution_mode",
    title: "Set Agent Team execution mode",
    description: "Update execution mode after evidence changes. Set NATIVE_SUBAGENTS immediately after a real native Codex subagent is successfully delegated; set SEQUENTIAL_ROLE_FALLBACK only after native delegation is unavailable or actually fails.",
    inputSchema: { type: "object", properties: {
      workspacePath: { type: "string" },
      executionMode: { type: "string", enum: ["NATIVE_SUBAGENTS", "SEQUENTIAL_ROLE_FALLBACK", "UNKNOWN"] },
      reason: { type: "string" }
    }, required: ["workspacePath", "executionMode"] },
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false }
  },
  {
    name: "agent_team_add_task",
    title: "Add Agent Team task",
    description: "Append a new task to an existing team, for example remediation, regression verification, or re-review work discovered after review.",
    inputSchema: { type: "object", properties: {
      workspacePath: { type: "string" },
  ...
  },
  {
    ...
    description: "Record a truthful native-agent/member status, current task, or summary.",
    inputSchema: { type: "object", properties: {
      workspacePath: { type: "string" }, memberId: { type: "string" },
      status: { type: "string", enum: ["idle", "working", "blocked", "done", "failed"] },
      currentTask: { type: ["string", "null"] }, summary: { type: "string" }
  ...
  },
  {
    ...
    description: "Update one task after real work occurs. The scheduler unlocks dependents, propagates dependency blocks, derives phase, and synchronizes members.",
    inputSchema: { type: "object", properties: {
      workspacePath: { type: "string" }, taskId: { type: "string" },
      status: { type: "string", enum: ["pending", "ready", "running", "blocked", "done", "failed"] },
      result: { type: "string" }, evidence: { type: "array", items: { type: "string" } }
  ...
  },
  {
    ...
    description: "Record an important orchestration event such as native delegation, fallback, review finding, remediation, verification, or blocker.",
    inputSchema: { type: "object", properties: {
      workspacePath: { type: "string" }, kind: { type: "string" }, message: { type: "string" },
      memberId: { type: ["string", "null"] }, taskId: { type: ["string", "null"] }
  ...
  },
  {
    ...
    description: "Render the live DSH-style Agent Team dashboard. If no team exists, render an uninitialized dashboard instead of failing.",
    inputSchema: { type: "object", properties: { workspacePath: { type: "string" } }, required: ["workspacePath"] },
    _meta: {
      ui: { resourceUri: TEMPLATE_URI }, "openai/outputTemplate": TEMPLATE_URI,
      "openai/toolInvocation/invoking": "Opening Agent Team dashboard…", "openai/toolInvocation/invoked": "Agent Team dashboard ready."
    },
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false }
  }
];
...
function callTool(name, args = {}) {
  if (name === "agent_team_create") {
    ...
    const state = {
      schemaVersion: 3,
      id: slug(args.name), name: requireString(args.name, "name"),
      ...
      planReviewState: "not_required", createdAt: now(), updatedAt: now(),
      members: normalizeMembers(args.members), tasks: normalizeTasks(args.tasks), events: []
    };
    validateTaskGraph(state);
  ...
  if (name === "agent_team_get") {
    const { root, state } = optionalWorkspaceState(args);
    if (!state) return textAndStructured("No Agent Team state exists yet. Create the team before substantial execution.", { workspacePath: root, statePath: statePath(root), initialized: false, team: null });
  ...
  if (name === "agent_team_set_execution_mode") {
    ...
    state.executionMode = args.executionMode;
  ...
  if (name === "agent_team_update_member") {
    ...
    if (args.status !== undefined) {
      ...
      member.status = args.status; member.statusSource = "manual";
    }
    if (Object.hasOwn(args, "currentTask")) {
      if (args.currentTask !== null && !state.tasks.some(task => task.id === args.currentTask)) throw new Error(`Unknown currentTask: ${args.currentTask}`);
...
function handle(id, method, params) {
  if (method === "initialize") {
    result(id, {
      ...
      instructions: "Maintain truthful DSH-style Agent Team state. Start execution mode UNKNOWN unless proven. After real native Codex delegation succeeds, immediately call agent_team_set_execution_mode with NATIVE_SUBAGENTS. Use SEQUENTIAL_ROLE_FALLBACK only when native delegation is unavailable or failed. Review findings that require fixes should add remediation, regression, and re-review tasks instead of falsely completing the team."
    });
    return;
  }
